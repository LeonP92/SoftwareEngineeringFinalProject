...
int DatabaseHelper::getMailId(QString fromUser, QString toUser, QString subject, QString message)
{
    ...
    if (sqlQuery.exec(QString("SELECT id FROM mail WHERE FromUser = '%1' AND ToUser = '%2' "
                              "AND Subject = '%3' AND Message = '%4'").arg(fromUser).
...
///
/// \brief DatabaseHelper::getUserId
/// \param fromUser
/// \param toUser
/// \return int
///
/// Function only used for testing to find out if user and password were
/// put into the database correctly.
int DatabaseHelper::getUserId(QString user, QString password)
{
    Q_ASSERT(getInitialized());
    ...
    if (sqlQuery.exec(QString("SELECT id FROM user WHERE User = '%1' AND Password = '%2' ")
                      .arg(user.toUpper()).arg(password))){
        sqlQuery.next();
        userId = sqlQuery.value(0).toInt();
    }else
        qDebug() << "ERROR: Could not load user ID. Return value is meaningless.";
